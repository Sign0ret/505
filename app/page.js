...
export default function Home() {
  ...
  const getBalances = async (publicKey) => {
    try {
      ...
      toast.error("Somethong went wrong getting the balance");
    }
  }
  const sendTransaction = async ( publicKey, receiver, amount) => {
    ...
    try {
      ...
      console.log("Solana",SOLANA_NETWORK);
      ...
      console.log("fromPub",fromPubkey);
      console.log("toPub",toPubkey);
    ...
    <div>
      ...
      {publicKey ? (
        <div>
          ...
          <br></br>
          <h1>Enviar una transacción a:</h1>
          <input
            className='h-8 w-72 mt-4 border-2 border-black'
            type="text"
            onChange={handleReceiverChange}
          />
          <h1>Cantidad de SOL a enviar:</h1>
          <input
            className='h-8 w-72 mt-4 border-2 border-black'
            type="text"
